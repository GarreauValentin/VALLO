...
sample_data(ps_mamm_R1)$concentration_tube <- as.numeric(as.character(sample_data(ps_mamm_R1)$concentration_tube))
sample_data(ps_mamm_R2)$concentration_tube <- as.numeric(as.character(sample_data(ps_mamm_R2)$concentration_tube))
ps_mamm_R1 <- prune_samples(!is.na(sample_data(ps_mamm_R1)$concentration_tube) & sample_data(ps_mamm_R1)$concentration_tube > 0, ps_mamm_R1)
ps_mamm_R2 <- prune_samples(!is.na(sample_data(ps_mamm_R2)$concentration_tube) & sample_data(ps_mamm_R2)$concentration_tube > 0, ps_mamm_R2)
contamdf.freq_mamm_R1 <- isContaminant(ps_mamm_R1, method="frequency", conc="concentration_tube")
contamdf.freq_mamm_R2 <- isContaminant(ps_mamm_R2, method="frequency", conc="concentration_tube")
...
plot_frequency(ps_filtered_mamm_R1, taxa_names(ps_filtered_mamm_R1)[c(1,3)], conc="concentration_tube") + 
  xlab("DNA Concentration (Qubit)")
plot_frequency(ps_filtered_mamm_R2, taxa_names(ps_filtered_mamm_R2)[c(1,3)], conc="concentration_tube") + 
  xlab("DNA Concentration (Qubit)")

table(contamdf.freq_mamm_R1$contaminant) #que des FALSE = pas de contaminants détéctés 
set.seed(100)
plot_frequency(ps_filtered_mamm_R1, taxa_names(ps_filtered_mamm_R1)[sample(which(contamdf.freq_mamm_R1$contaminant),3)], conc="concentration_tube") +
...
set.seed(100)
plot_frequency(ps_filtered_mamm_R2, taxa_names(ps_filtered_mamm_R2)[sample(which(contamdf.freq_mamm_R2$contaminant),3)], conc="concentration_tube") +
  xlab("DNA Concentration (Qubit)") #plot impossible car pas de contaminants 
