...
#Table OTUs
data_plant_R2_prev <- read.table(file = "merge_otu_taxo_plantglobalR2prev.csv", header = TRUE, sep = ",", row.names = "X")
data_plant_R2_freq <- read.table(file = "merge_otu_taxo_plantglobalR2freq.csv", header = TRUE, sep = ",", row.names = "X")

#formarrage donnéés
data_plant_R2_prev <- subset(data_plant_R2_prev, Subgroups != "")
data_plant_R2_freq <- subset(data_plant_R2_freq, Subgroups != "")


#plant_R1_prev
# Initialiser un tableau vide pour stocker le résultat
result_plant_R2_prev <- data.frame()

# Boucle sur chaque colonne de données pour extraire les informations taxonomiques
for (col in colnames(data_plant_R2_prev)[!colnames(data_plant_R2_prev) %in% c("Subgroups", "Infragroups", "Class", "Order", "Family", "Genus", "Species")]) {
  
  # Filtrer les lignes où la valeur est > 0 dans la colonne actuelle
  rows_with_data <- data_plant_R2_prev[data_plant_R2_prev[[col]] > 0, ]
  ...
  if (nrow(rows_with_data) > 0) {
    ...
    result_plant_R2_prev <- rbind(result_plant_R2_prev, tax_info)
  }
}

#noms lignes 
result_plant_R2_prev$Sample <- sub("_.*", "", rownames(result_plant_R2_prev))
#merge données extérieurs et resultats
result_plant_R2_prev <- merge(result_plant_R2_prev, extdata, by.x = "Sample", by.y = "Echantillons", all.x = TRUE)
...
#ANALYSES DONNEES
ggplot(data = result_plant_R2_prev, aes(x = Altitude_.m., y = Species)) +
